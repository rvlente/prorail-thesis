...
{
public:
    S2ShapeIndexExperimentRunner(const char *name) : BaseExperimentRunner(name){};

private:
    std::vector<std::unique_ptr<S2PointVectorShape>> load_geometry(const char *file_path, std::function<void(size_t, size_t)> progress)
    {
    ...
    std::vector<S2ShapeRangeQuery> load_range_queries(const char *file_path, std::function<void(size_t, size_t)> progress)
    {
